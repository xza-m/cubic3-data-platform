...
def route(ctx: typer.Context, question: Annotated[str, typer.Argument()]) -> None:
    call_and_emit(ctx, "POST", _ROUTE, json_body={"question": question})


@app.command("extract", help="L1 意图理解产物（grounded，取自 route）")
def extract(ctx: typer.Context, question: Annotated[str, typer.Argument()]) -> None:
    ...
    call_project_emit(ctx, "POST", _ROUTE, json_body={"question": question}, project=_project)


@app.command("answerability", help="可回答性门控（answerable/out_of_coverage/...）")
def answerability(ctx: typer.Context, question: Annotated[str, typer.Argument()]) -> None:
    def _project(data: dict) -> dict:
        bi = data.get("business_intent") or {}
