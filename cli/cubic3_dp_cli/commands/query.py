...
def compile_dsl(
    ctx: typer.Context,
    dsl: Annotated[str, typer.Option("--dsl", help="QueryDSL JSON（内联 / @file / -）")],
) -> None:
...
def plan(ctx: typer.Context, question: Annotated[str, typer.Argument()]) -> None:
    call_and_emit(ctx, "POST", "/api/v1/semantic-router/plan", json_body={"question": question})


@app.command("explain", help="NL → 编译预览 SQL（preview-only，不碰 gateway）")
def explain(ctx: typer.Context, question: Annotated[str, typer.Argument()]) -> None:
    call_and_emit(ctx, "POST", "/api/v1/semantic-router/execute-plan-preview", json_body={"question": question})
