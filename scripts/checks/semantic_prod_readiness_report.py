...
def build_report(env: Mapping[str, str]) -> dict[str, Any]:
    ...
    checks = {
        "baseline_fingerprint": {
            "status": "ready" if database_url else "blocked",
        ...
        "fixture_cleanup": {
            "status": "ready" if fixture_namespace and database_url else "blocked",
            "required": ["DATABASE_URL", "SEMANTIC_FIXTURE_NAMESPACE"],
            "evidence": "semantic-fixture-cleanup",
        },
        "postgres_concurrency": {
            "status": "ready" if _is_postgresql_url(database_url) else "blocked",
