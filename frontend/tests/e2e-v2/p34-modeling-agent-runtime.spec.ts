...
//
// P34 - Copilot 只消费平台 runtime 状态，Codex 启动/联通留在平台设置页。
...
test('Copilot 不展示 runtime 切换或 Codex 启动，平台设置页负责启动动作 @smoke @p34', async ({ page }) => {
  let startRequested = false

  await page.route('**/api/v1/agent-runtime/providers/codex_app_server/start', async (route) => {
    startRequested = true
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await gotoV2(page, `/semantic/modeling-copilot/${SESSION_ID}`)
  ...
  await expect(page.getByText('Codex app-server', { exact: true })).toBeVisible()
  await expect(page.getByText('ws://127.0.0.1:8799')).toBeVisible()
  ...
  return {
    ...
    providers: [
      ...
      },
      {
        ...
        operations: ['test', 'start', 'restart', 'capabilities', 'logs'],
        details: {
          transport: 'ws',
...
function reviewPayload() {
  return {
    ...
    data_agent_consumption: {
      runtime_truth: false,
      status: 'draft',
      hint: '未发布前不进入正式 Data Agent runtime。',
    },
  }
}
