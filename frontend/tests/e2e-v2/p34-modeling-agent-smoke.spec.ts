...
function sessionPayload(phase: FlowPhase) {
  ...
  const confirmed =
    phase === "confirmed" || phase === "saved" || phase === "published";
  ...
  return {
    ...
    workbench_state: {
      ...
      validation_summary: [],
      proposal_summary: hasProposal
        ? { id: "proposal_agent_1", status: "validated" }
        : {},
...
function sessionPayloadWithReleasePreview(phase: FlowPhase = "saved") {
  ...
  return {
    ...
    workbench_state: {
      ...
      release_preview: {
        ...
        compiled_sql:
          "select comment_school_name, count(report_id) as total_count from df_cb_258187.dwd_interaction_comment_reports_df group by comment_school_name",
        release_diff: {
          added: ["cube.dwd_interaction_comment_reports_df"],
          changed: ["metric.student_comment_total_count"],
          removed: [],
        },
        impact_summary: {
          affected_assets: ["student_comment_total_count"],
        ...
        gateway_validation: {
          ...
          message: "Gateway 未配置，语义中心编译预演已通过。",
        },
        consumer_validation: {
          status: "passed",
          samples: [
...
function reviewPayload(phase: FlowPhase) {
  ...
  return {
    ...
    source_evidence: (session.workbench_state as Record<string, unknown>)
      .source_evidence,
    trace_state: {
      events: analyzed
        ? [
      ...
      steps: [
        {
          id: "spec",
          label: "语义草案完整",
          status: analyzed ? "passed" : "blocked",
          description: "语义草案已生成",
        },
        {
          id: "blockers",
          ...
          description: blockers.length > 0 ? "仍有阻塞项" : "阻塞项已处理",
        },
        {
    ...
    post_publish_validation: {
      ...
      result_summary: published
        ? "语义中心发布快照已生成，消费者可继续验证。"
        : "语义资产发布后，消费者可基于语义中心发布快照验证。",
    },
  };
}
  ...
  return {
    ...
    workbench_state: {
      ...
      readiness: {
        ...
        reasons: generated
          ? ["ready_to_save"]
          : ["source_candidate_confirmation_required", "spec_not_generated"],
      },
      raw_spec: generated
        ? {
            spec_version: "v1",
            ...
            cube: {
              ...
              dimensions: [
                { name: "class_id", type: "string", expr: "class_id" },
              ],
              measures: [
                { name: "active_count", type: "count", sql: "COUNT(*)" },
              ],
            },
            ontology: {
              ...
              metrics: [
                {
                  name: "class_activity",
                  title: "班级活跃度",
                  measure_refs: ["dwd_class_activity_df.active_count"],
                },
              ],
            },
          }
      ...
      advanced_refs: {
        ...
        candidate_source_table: generated
          ? "dw.dwd_class_activity_df"
          : undefined,
      },
    },
    tool_traces: generated
      ? [
          ...
          },
          {
            ...
            status: "completed",
            summary: "已确认数据来源 dw.dwd_class_activity_df",
          },
          {
            tool: "generate_semantic_draft",
            status: "completed",
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(
    ...
    async (route, request) => {
      if (request.method() === "POST") {
        ...
        const userGoal = String(payload.user_goal ?? "");
        if (payload.entry_type === "table_known") {
          expect(userGoal).toContain("dwd_learning_activity_df");
          expect(payload.workbench_context).toEqual(
          ...
          expect(userGoal).toBe(QUESTION);
          expect(payload.entry_type).toBe("business_question");
        }
        phase = "created";
        releasePreviewReady = false;
        await route.fulfill({
          status: 200,
          contentType: "application/json",
  ...
  await page.route(
    ...
    async (route) => {
      ...
      if (message.includes("dwd_learning_activity_df")) {
        phase = "saved";
      ...
      await route.fulfill({
        ...
        body: JSON.stringify(
          envelope(
            releasePreviewReady
              ? sessionPayloadWithReleasePreview("saved")
              : sessionPayload("analyzed"),
          ),
        ),
      });
    },
  ...
  await page.route(
    "**/api/v1/semantic/modeling-copilot/sessions/session_agent_1/release-preview",
    async (route, request) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(envelope(sessionPayloadWithReleasePreview())),
      });
    },
  );
  ...
  await page.route(
    ...
    async (route) => {
      ...
      if (spec && typeof spec === "object") {
        ...
        if (typeof business.sensitivity_level === "string")
          patchedSpecSensitivity = business.sensitivity_level;
      }
          ...
          raw_spec:
            spec && typeof spec === "object"
              ...
              : {
                  ...
                  cube: {
                    ...((
                      baseline.workbench_state.raw_spec as
                        | Record<string, unknown>
                        | undefined
                    )?.cube as Record<string, unknown> | undefined),
                    ...cube,
                  },
                },
  ...
  await page.route(
    ...
    async (route, request) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify(
          envelope(
            releasePreviewReady && phase === "saved"
              ? sessionPayloadWithReleasePreview("saved")
              : sessionPayload(phase),
          ),
        ),
      });
    },
...
test("P34 语义建设工作台快速模式从业务问题应用并发布语义 @smoke @p34", async ({
  page,
}) => {
  await gotoV2(page, "/semantic/modeling-workbench/quick");

  // 空态：语义冷启动引导
  await expect(page.getByText("从数仓数据建设可发布的语义资产")).toBeVisible();
  await expect(page.getByText(/发布到语义中心/)).toBeVisible();
  ...
  // Workbench-first：主流程承接确认与字段候选主画布，右侧 artifact panel 保留兼容审阅。
  await expect(page.getByTestId("chat-workspace")).toBeVisible();
  const artifacts = page.getByTestId("artifact-panel");
  const fieldCanvas = page.getByTestId("field-candidate-main-canvas");
  await expect(artifacts).toBeVisible();
  const stepper = page.getByTestId("semantic-builder-stepper");
  await expect(stepper).toBeVisible();
  await expect(
    stepper.locator('[aria-current="step"]').getByText("语义草案"),
  ).toBeVisible();
  const aiActions = page.getByTestId("builder-ai-actions");
  await expect(aiActions.getByText("AI 建模助手")).toBeVisible();
  await aiActions.getByRole("button", { name: /生成语义草案/ }).click();
  await expect(page.getByLabel("建模目标")).toHaveValue(/发布到语义中心/);
  await page.getByLabel("建模目标").fill(QUESTION);
  await expect(
    page.getByTestId("copilot-run-state").getByText("当前状态：等待你确认口径"),
  ).toBeVisible();
  await expect(
    page
      .getByTestId("copilot-run-state")
      .getByText("在建设主流程的确认卡片里处理；后台没有继续运行。"),
  ).toBeVisible();
  await expect(fieldCanvas).toBeVisible();
  await expect(fieldCanvas.getByText("字段候选主画布")).toBeVisible();
  await expect(
    fieldCanvas.getByRole("table", { name: "字段候选审阅" }),
  ).toBeVisible();
  await expect(fieldCanvas.getByText("评论数")).toBeVisible();
  await expect(fieldCanvas.getByText("report_id")).toBeVisible();
  await expect(fieldCanvas.getByText("92%")).toBeVisible();
  await expect(
    artifacts.getByTestId("proposal-review-workbench"),
  ).toBeVisible();
  await expect(artifacts.getByText("资产审阅")).toBeVisible();
  await expect(
    artifacts.getByRole("button", { name: /^字段候选$/ }),
  ).toBeVisible();
  await artifacts.getByRole("button", { name: /^字段候选$/ }).click();
  await expect(artifacts.getByTestId("field-candidate-review")).toBeVisible();
  await expect(artifacts.getByText("字段候选审阅")).toBeVisible();
  await artifacts.getByRole("button", { name: /^建设摘要$/ }).click();
  await expect(
    artifacts
      .getByTestId("artifact-guidance")
      .getByText(/流程已阻塞：学校维度口径待确认/),
  ).toBeVisible();
  await expect(
    artifacts.getByRole("button", { name: /^语义草案$/ }),
  ).toBeVisible();
  await expect(
    artifacts.getByRole("button", { name: /^审计记录$/ }),
  ).toBeVisible();
  await expect(artifacts.getByTestId("artifact-run-state")).toHaveCount(0);
  await expect(
    artifacts.getByRole("button", { name: /使用推荐|保存|发布到语义中心/ }),
  ).toHaveCount(0);

  const review = artifacts.getByTestId("proposal-review-workbench");
  await expect(review).toBeVisible();
  await expect(review.getByText("建设摘要").first()).toBeVisible();
  await expect(review.getByText("为什么卡住").first()).toBeVisible();
  await expect(review.getByText(/变更摘要/).first()).toBeVisible();
  await expect(
    review.getByRole("button", {
      name: /使用推荐|保存|发布到语义中心|调整语义配置/,
    }),
  ).toHaveCount(0);
  await expect(
    page.getByText("dwd_interaction_comment_reports_df").first(),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("dwd_interaction_comment_reports_df.total_count").first(),
  ).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: /生成语义资产/ })).toHaveCount(
    0,
  );

  // 可选草稿确认：接受语义草稿不会触发 LLM，也不会绕过真正的口径阻塞。
  await page
    .getByTestId("chat-workspace")
    .getByRole("button", { name: "接受草稿" })
    .click();
  await expect(page.getByText("语义草稿（已接受）").first()).toBeVisible();
  await expect(page.getByRole("button", { name: /生成语义资产/ })).toHaveCount(
    0,
  );

  // 阻断确认：主操作保留在建设主流程卡片里，右侧只做辅助审阅
  await page
    .getByTestId("chat-workspace")
    .getByRole("button", { name: "使用推荐" })
    .click();

  // confirm 完后阻断确认卡片消失，主链路进入生成语义资产。
  await expect(page.getByText("需要你确认")).toHaveCount(0);
  await expect(
    page
      .getByTestId("chat-next-action")
      .getByRole("button", { name: /生成语义资产/ }),
  ).toBeEnabled();
  await expect(review.getByText("可以生成语义资产").first()).toBeVisible();

  // 我的判断依据折叠
  await page.getByRole("button", { name: /我的判断依据/ }).click();
  await expect(page.getByText(QUESTION).first()).toBeVisible();

  // 建设主流程中打开语义草案编辑：右侧切到语义草案 tab，改配置，验证 PATCH 已发出
  await page
    .getByTestId("chat-workspace")
    .getByRole("button", { name: /在右侧编辑语义配置/ })
    .click();
  await expect(page.getByTestId("artifact-spec-panel")).toBeVisible();
  await expect(artifacts.getByLabel("完整语义草案")).toBeVisible();
  const rawSpecEditor = artifacts.getByLabel("完整语义草案");
  const fullSpec = JSON.parse(await rawSpecEditor.inputValue()) as Record<
    string,
    unknown
  >;
  fullSpec.business = { subject: "学生评论", sensitivity_level: "internal" };
  await rawSpecEditor.fill(JSON.stringify(fullSpec, null, 2));
  await artifacts.getByRole("button", { name: /保存高级语义配置/ }).click();
  // @ts-expect-error - test fixture 暴露
  expect(page._getPatchedSpecSensitivity()).toBe("internal");
  await expect(
    page
      .getByTestId("artifact-spec-panel")
      .locator('[data-testid="cube-editor"]'),
  ).toHaveCount(0);
  await artifacts.getByRole("button", { name: /让 AI 调整语义配置/ }).click();
  ...
  // 右侧来源证据 tab：展示源表证据边界
  await artifacts.getByRole("button", { name: /^来源证据$/ }).click();
  ...
  await artifacts.getByRole("button", { name: /^审计记录$/ }).click();
  await expect(page.getByTestId("artifact-trace-panel")).toBeVisible();
  await expect(
    page
      .getByTestId("artifact-trace-panel")
  ...
  // 右侧可用性验证 tab：展示草稿验证边界
  await artifacts.getByRole("button", { name: /^可用性验证$/ }).click();
  const previewPanel = page.getByTestId("artifact-preview-panel");
  await expect(
    previewPanel.getByRole("heading", { name: "草稿可用性验证" }),
  ).toBeVisible();
  await expect(
    previewPanel.getByText("不写入语义中心发布快照", { exact: true }),
  ).toBeVisible();
  await expect(
    previewPanel.getByText(/基于语义中心发布快照验证/).first(),
  ).toBeVisible();

  // 生成语义资产（save proposal）
  await page
    .getByTestId("chat-next-action")
    .getByRole("button", { name: /生成语义资产/ })
    .click();
  ...
  // 第二步：先经过语义中心编译预演，再发布到语义中心。
  await artifacts.getByRole("button", { name: /^建设摘要$/ }).click();
  await artifacts.getByRole("button", { name: /运行发布预演/ }).click();
  await expect(page.getByText("发布预演已生成").first()).toBeVisible();
  await expect(page.getByText("语义中心编译通过").first()).toBeVisible();
  await expect(
    page.getByRole("button", { name: /发布到语义中心/ }),
  ).toBeEnabled();
  await page.getByRole("button", { name: /发布到语义中心/ }).click();
  await expect(page.getByText("语义已发布").first()).toBeVisible();
  await expect(page.getByText("已发布到语义中心").first()).toBeVisible();
  await artifacts.getByRole("button", { name: /^建设摘要$/ }).click();
  await expect(page.getByText("发布到语义中心检查").first()).toBeVisible();
  await expect(page.getByText("发布门禁已通过").first()).toBeVisible();
  await expect(page.getByText("发布后消费者验证").first()).toBeVisible();
...
test("P34 候选来源确认后确定性生成 spec @smoke @p34", async ({ page }) => {
  ...
  await page.route(
    ...
    async (route) => {
      ...
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify(
          envelope(sourceCandidateSessionPayload("generated")),
        ),
      });
    },
  );
  ...
  await page.route(
    ...
    async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify(
          envelope({
            session_id: "session_source_candidate_1",
            status: phase,
            blockers: [],
          }),
        ),
  ...
  await expect(
    page.getByTestId("chat-workspace").getByText("推荐数据来源"),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByTestId("copilot-run-state")
      .getByText("当前状态：等待你确认数据来源"),
  ).toBeVisible();
  await expect(
    page.getByTestId("copilot-run-state").getByText(/后台没有继续运行/),
  ).toBeVisible();
  await expect(
    page.getByTestId("artifact-guidance").getByText("流程已阻塞：确认数据来源"),
  ).toBeVisible();

  await page.getByRole("button", { name: /使用此来源/ }).click();

  await expect(
    page.getByText("已使用 dw.dwd_class_activity_df 生成可审阅 spec。"),
  ).toBeVisible();
  await expect(
    page
      .getByTestId("copilot-run-state")
      .getByText("当前状态：草稿已生成，可预演"),
  ).toBeVisible();
  await expect(page.getByText("语义草稿").first()).toBeVisible();
  await page
    .getByTestId("artifact-panel")
    .getByRole("button", { name: /^语义草案$/ })
    .click();
  await expect(page.getByTestId("artifact-spec-panel")).toBeVisible();
  await expect(page.getByLabel("完整语义草案")).toContainText(
    "dwd_class_activity_df",
  );
});

test("P1 Build Project 批量语义建设生成真实候选队列 @smoke @p34", async ({
  page,
}) => {
  const buildProject = {
    id: "build-learning",
  ...
  await page.route(
    "**/api/v1/semantic/modeling-workbench/projects",
    async (route, request) => {
      if (request.method() === "POST") {
        const payload = request.postDataJSON() as Record<string, unknown>;
        expect(payload.name).toBe("学情分析");
        expect(payload.business_domain).toBe("学情分析");
        await route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify(
  ...
  await page.route(
    ...
    async (route, request) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(envelope(buildProject)),
      });
    },
  );
  await page.route(
  ...
  await expect(
    page.getByRole("button", { name: "标记重复" }).first(),
  ).toBeVisible();
  await expect(page.getByText("1 个候选资产包")).toBeVisible();
  ...
  await expect(page.getByText("批量候选上下文")).toBeVisible();
  ...
  await expect(page.getByText("Cube 层与本体锚定")).toHaveCount(0);
  await expect(page).not.toHaveURL(/\/semantic\/modeling-copilot\//);
  await page.getByRole("button", { name: "发送" }).click();

  await expect(page).toHaveURL(
    /\/semantic\/modeling-workbench\/build-learning\/candidate\/build-learning%3Afact%3Adwd-learning-activity-df\?sessionId=session_agent_1$/,
  );
  await expect(page).not.toHaveURL(/\/semantic\/modeling-copilot\//);
  await expect(page.getByText("字段候选主画布").first()).toBeVisible();
  await expect(page.getByText("可发布检查").first()).toBeVisible();
  await expect(page.getByText("语义草案完整性").first()).toBeVisible();
