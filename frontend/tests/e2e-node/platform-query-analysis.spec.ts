...
test('查询中心主入口会收口到查询分析中心，旧列表入口重定向并保留意图', async ({ page }) => {
  ...
  expect(new URL(page.url()).searchParams.get('legacy')).toBe('my')
  await expect(page.getByRole('button', { name: '运行' })).toBeVisible()
  await expect(page.getByRole('button', { name: '展开模版库' })).toBeVisible()
  await expect(page.getByPlaceholder('搜索模版...')).toHaveCount(0)

  await page.goto('/queries/history?sourceId=1&name=%E8%AF%BE%E5%A0%82%E8%BF%9B%E5%BA%A6%E5%88%86%E6%9E%90', { waitUntil: 'domcontentloaded' })
  await expect(page.getByTestId('query-center-dashboard-layout')).toBeVisible()
  expect(new URL(page.url()).pathname).toBe('/queries')
  expect(new URL(page.url()).searchParams.get('legacy')).toBe('history')
  expect(new URL(page.url()).searchParams.get('sourceId')).toBe('1')
  expect(new URL(page.url()).searchParams.get('name')).toBe('课堂进度分析')
  ...
  await expect(page.getByTestId('query-center-dashboard-layout')).toBeVisible()
  expect(new URL(page.url()).pathname).toBe('/queries')
  expect(new URL(page.url()).searchParams.get('legacy')).toBe('scheduled')
  await expect(page.getByRole('button', { name: '展开模版库' })).toBeVisible()
  await expect(page.getByPlaceholder('搜索模版...')).toHaveCount(0)
})

test('查询构建旧入口并回主工作台，智能问数仍保留独立工作区', async ({ page }) => {
  ...
  await page.goto('/queries/visual', { waitUntil: 'domcontentloaded' })
  await expect(page.getByTestId('query-center-dashboard-layout')).toBeVisible()
  expect(new URL(page.url()).pathname).toBe('/queries')
  expect(new URL(page.url()).searchParams.get('legacy')).toBe('visual')
  await expect(page.getByText('模版库')).toBeVisible()
