...
test('数据源页触发真实目录同步反馈，且不再显示历史摘要卡', async ({ page }) => {
  ...
  await expect(page.getByText('管理所有数据库连接配置')).toBeVisible()
  await expect(page.getByText('总数据源')).toBeVisible()
  await expect(page.getByText('活跃')).toBeVisible()
  await expect(page.getByText('未激活')).toBeVisible()
  await expect(page.getByText('教学 PostgreSQL')).toBeVisible()
  await expect(page.getByText('主学习业务库')).toBeVisible()
  ...
  await syncRequest
  const notificationRegion = page.getByRole('region', { name: 'Notifications (F8)' })
  await expect(notificationRegion.getByText('目录同步已触发')).toBeVisible()
  await expect(notificationRegion.getByText('目录刷新任务已加入队列，卡片会自动刷新同步状态。')).toBeVisible()
  ...
  await expect(page.getByText('物理数据集')).toBeVisible()
  await expect(page.getByText('postgresql')).toBeVisible()
  await expect(page.getByText('物理表 / 来源对象')).toBeVisible()
  await expect(page.getByText('数据预览')).toHaveCount(0)
  await expect(page.getByRole('heading', { name: '血缘分析' })).toHaveCount(0)
  await expect(page.getByRole('heading', { name: '影响分析' })).toHaveCount(0)
  await expect(page.getByRole('heading', { name: '质量评分' })).toHaveCount(0)
  ...
  await expect(notificationRegion.getByText('保存失败')).toBeVisible()
  await expect(notificationRegion.getByText('没有权限修改数据集')).toBeVisible()
