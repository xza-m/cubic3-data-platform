...
const TASK_TYPE_OPTIONS: Array<{ value: CreateTaskPayload['task_type']; label: string }> = [
    ...
    return (
      <div className="flex flex-1 items-center justify-center p-8">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <div className="text-sm font-medium" style={{ color: 'var(--text-1)' }}>创建成功</div>
          <p className="max-w-sm text-xs" style={{ color: 'var(--text-3)' }}>
            任务 <strong>{taskName}</strong> 已创建。
          </p>
    ...
    <div className="flex flex-1 flex-col overflow-hidden p-4">
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b px-4 py-3" style={{ borderColor: 'var(--border)' }}>
          ...
          <span className="text-xs font-medium" style={{ color: 'var(--text-1)' }}>新建提取任务</span>
        </div>
          ...
          <div className="mx-auto max-w-xl space-y-5 p-6">
            ...
            <Field label="任务名称" required>
              <input
                ...
                required
                placeholder="如 dwd_order 日增量提取"
                style={inputStyle}
              />
            </Field>

            <Field label="目标数据集" required>
              <select
                value={datasetId ?? ''}
            ...
            <Field label="启用">
              <label className="flex cursor-pointer items-center gap-2 text-xs">
                ...
                />
                <span style={{ color: 'var(--text-2)' }}>创建后立即启用</span>
              </label>
            </Field>

            {createTask.isError && (
              <p className="text-xs" style={{ color: 'var(--danger)' }}>
                {createTask.error instanceof Error ? createTask.error.message : '创建失败，请重试'}
              </p>
            )}
