...
- `Workbench` -> `/semantic/workbench`
- `Inventory` -> `/semantic/cubes`、`/semantic/domains`
- `Canvas` -> `/semantic/modeling`、`/semantic/domains/:id`

`/semantic/cubes/new`、`/semantic/cubes/:name/edit`、`/semantic/tools` 只保留兼容重定向，都会回流到 `/semantic/workbench`。`View`、`Recipe`、`Schema Drift` 等能力继续挂靠在现有页面模型中，不单独升格为一级导航。
...
- `Workbench` 负责完整开发流，`Inventory` 负责正式资产浏览，职责比“列表里新建 + 工具页里再编辑”更清楚
...
- 新的语义能力优先落到当前 `Workbench / Inventory / Canvas` 页面模型中，而不是直接新增一级页
- `semantic/modeling` 作为建模入口页，属于 `Canvas` 流程，不应演化成独立大模块
- `/semantic/workbench` 是唯一开发主场；`/semantic/cubes` 不再承担创建入口
