...
当前实现已经形成平台内 `AgentInferenceRuntimeService`、OpenAI-compatible adapter、语义建模与数据资产两个 consumer、最小 SQL run / artifact 仓储、只读查询 API，以及平台级 action binding / provider status / Codex SDK 连接测试。Codex 后台任务当前通过 `CodexSdkClient` 调用 SDK，不暴露底层 transport。
...
- Phase 4 Codex SDK adapter：已实现 SDK client、异步 run lifecycle、event / artifact 查询和权限校验；真实 provider 通信通过 `CodexSdkClient`。
- Phase 5 Data Asset Agent App：已实现 `asset.field.infer_semantics` 第二 consumer，数据资产底座可通过同一 runtime contract 生成字段语义候选。
- Runtime Management MVP：已实现 `ActionRuntimeBindingRegistry`、`AgentRuntimeManagementService` 与 `CodexSdkClientRegistry`；业务 action 可以固定 OpenAI、固定 Codex SDK 或在专家 action 下显式允许选择，Copilot 默认只展示状态，不暴露 runtime 切换器。
...
- 管理 API：`/api/v1/agent-runtime/providers/status`、`/api/v1/agent-runtime/actions/<action>/binding`、`/api/v1/agent-runtime/providers/<runtime>/test`、`logs / capabilities` 只暴露状态、绑定、连接测试和诊断能力；`start / stop / restart` 对 Codex SDK 返回禁用。
...
- Codex SDK lifecycle 支持 run / event / artifact contract；真实 provider 通信通过 SDK client，不再依赖 loopback WebSocket transport。
...
- 同时支持 OpenAI Agents SDK runtime 与 Codex SDK runtime。
...
- 不把 Codex SDK 当成普通 LLM provider。
...
### 2.4 业务 action 与 Runtime 切换边界

平台级 Runtime 不等于所有业务页面都暴露 runtime 切换器。业务模块只声明 action 意图，平台通过 `ActionRuntimeBindingRegistry` 解析默认 runtime、允许 runtime 和是否可以展示选择器：
...
| `semantic.modeling.review_proposal` / `repair_*` / `audit_*` | `codex_sdk` | 不展示 | 长上下文复审、修复建议和 workspace artifact，固定 Codex runtime |
| `semantic.modeling.expert_debug` | `openai_compatible`，可选 `codex_sdk` | 仅专家模式展示 | 明确需要对比 runtime 行为时才允许手动选择 |

当 action 固定为 OpenAI 时，前端或请求体传入 `preferred_runtime=codex_sdk` 会被 router 拒绝，避免把技术实现选择暴露给普通数据开发人员。Copilot 主流程只展示 AI Runtime 状态；只有进入固定 Codex 的 review / repair / audit 能力，才需要展示 Codex 连接或诊断入口。当前 Copilot 在已有 Proposal 且 `review_proposal` 需要 Codex 连接时展示轻量提示；旧启停 API 只保留为 fail-closed 防线，不承接启动命令。
...
flowchart TD
  ...
  subgraph Adapters["Runtime Adapter 层"]
    ...
    Codex["Codex SDK Adapter<br/>Workspace Runtime"]
  end
...
| Runtime Adapter 层 | 调用 OpenAI Agents SDK、LLM API 或 Codex SDK | 业务决策、平台状态修改 |
...
它可以复用 Codex SDK 的工作区能力，但不能直接改 DataWorks 生产任务。修复产物应作为建议或 patch artifact 进入人工确认链路。
...
| Codex SDK Runtime | 工作区型 agent runtime | 长上下文、文件 / 命令 / artifact、复杂复审、修复建议 | 高频低延迟主对话、直接写生产状态 |

命名约束：

- `OpenAIAgentsSdkRuntimeAdapter` 必须表示真实 Agents SDK 接入。
- 仅使用 Chat Completions 协议的适配器应命名为 `OpenAICompatibleLLMRuntimeAdapter`。
- `CodexSdkRuntimeAdapter` 表示接入 Codex SDK 的工作区 runtime。
...
- 生命周期管理补齐 `submit / poll / cancel / expire / read_result`；Codex SDK 和批量审计默认异步执行。
...
| review / repair / audit | Codex SDK | 长上下文与复杂推理 |
| batch / workspace / file_artifact | Codex SDK | 需要工作区和 artifact |
...
- Codex SDK 不可用时，不自动降级为 OpenAI 主链复审，除非 action 明确允许 `fallback_runtime=openai`。
...
- Codex SDK 使用项目级隔离工作区和独立 `AGENT_CODEX_RUNTIME_ROOT`。
...
Codex SDK 当前只读取以下 `AGENT_CODEX_*` 字段；默认不启用真实 SDK 调用。主链路由 `CodexSdkClient` 调用 SDK，不再暴露底层 transport 选择。
Docker 后端镜像会在构建阶段通过 `openai_codex_sdk.install.install_codex` 安装 Linux Codex CLI；本地 compose 会透传这些环境变量，但默认仍关闭 SDK provider。

```text
AGENT_CODEX_ENABLED=false
AGENT_CODEX_UI_MANAGED=false
...
AGENT_CODEX_SANDBOX=read-only
AGENT_CODEX_PATH=/optional/path/to/codex
AGENT_CODEX_BASE_URL=
AGENT_CODEX_TIMEOUT_SECONDS=10
AGENT_CODEX_MAX_CONCURRENCY=2
```

Codex SDK live smoke 的测试开关是 opt-in 验证字段，不属于默认运行时配置：
...
Codex SDK 验证通过后的运行态以本地目录为主。配置只暴露项目根和 runtime 根目录，session / thread / turn / artifact 目录由平台按 `project_id / session_id / thread_id / turn_id / run_id` 派生，避免用户手动配置多层路径。

`AGENT_CODEX_UI_MANAGED` 只控制前端是否展示连接测试和诊断入口。当前实现不接受前端传入启动命令；`start / stop / restart` 对 Codex SDK 返回禁用并记录审计。`providers/status` 不在列表刷新时触发 SDK healthcheck，只合成配置状态和最近一次同配置连接测试审计；真实 SDK 是否可用以 `test_provider("codex_sdk")` 和 opt-in SDK live smoke 为准。

### 15.1 Command Provider 与 Codex Runtime 的区别

普通 command provider 的抽象是“一次命令执行”，典型配置是 `command / args / cwd / timeout`，典型结果是 `exit_code / stdout / stderr`。它适合 `sqlfluff`、`pytest`、`odpscmd` 或一次性脚本，不适合承载 Codex SDK。

Codex SDK 的抽象是“agent 工作区会话”，核心对象是 `project / session / thread / turn / run / artifact`。平台需要管理长任务状态、事件流、artifact、权限和可恢复 trace，而不是只等待一个子进程输出。

| 维度 | 普通 command provider | Codex SDK runtime |
|---|---|---|
| 核心对象 | command、args、cwd、timeout | project、session、thread、turn、run、artifact |
| 调用形态 | 启动一次进程并等待结果 | 通过 SDK client 创建 thread / run 并桥接为平台异步生命周期 |
...
因此 `AGENT_CODEX_*` 不设计 `COMMAND / ARGS` 作为主配置。平台只通过 SDK client 与 Codex runtime 通信；底层运行细节不作为平台可配置 transport。

### 15.2 Codex SDK Provider Contract

Codex SDK provider 必须通过当前服务环境显式配置项目根、sandbox 和超时，不允许业务页面选择底层 transport，也不在本项目内管理多个外部 Codex 项目环境。平台只关心 `healthcheck / capabilities / submit / poll / cancel / stream_events / collect_artifacts`。

| 配置 | 说明 |
|---|---|
| `AGENT_CODEX_PROJECT_ROOT` | 当前服务自己的 Codex SDK 工作目录 |
| `AGENT_CODEX_RUNTIME_ROOT` | 平台 run / artifact 运行态根目录 |
| `AGENT_CODEX_SANDBOX` | SDK sandbox，默认 `read-only` |
| `AGENT_CODEX_PATH` | 可选 Codex CLI 路径覆盖 |
| `AGENT_CODEX_BASE_URL` | 可选 OpenAI base URL |
| `AGENT_CODEX_MAX_CONCURRENCY` | 当前服务 SDK provider 并发 run 上限 |
...
- `healthcheck` 与 `capabilities` 是平台侧语义方法，不要求映射为 HTTP path。
- `submit / poll / cancel / stream_events / collect_artifacts` 由 `CodexSdkClient` 桥接到平台异步 run lifecycle。
- SDK 原生 provider id 只保存在 `provider_ref`，不进入平台领域 id。
...
| `AgentInferenceRuntimeRun` | `runs/{run_id}/run.json` 与 SDK run 状态 |
...
- `project`：当前服务运行时的目录 namespace，记录 `project_id`、`project_root`、runtime 版本和 SDK 能力摘要；它不是多项目环境注册表。
- `session`：对应业务产品会话，例如一次建模 Copilot session；保存调用方、principal、业务对象和默认 context pin。
- `thread`：对应同一 session 下的 agent 对话 / 推理线程；承接多轮上下文和 Codex SDK thread 标识。
...
| SDK 原生 thread/run 状态 | Codex SDK | 仅由 adapter 同步为 provider manifest |
...
- 目标演进 stale run 恢复规则：`running` 超过 `AGENT_CODEX_MAX_RUNTIME_SECONDS` 且 SDK 查询不到活跃 provider run 时，标记为 `timeout` 或 `expired`，并记录 recovery event。
...
- SDK 生成的文件进入平台 artifact store 时必须复制或登记为平台 manifest，不能直接暴露 provider 临时路径。
...
- `read_events` 通过平台运行态事件页读取增量状态；底层 Codex SDK 事件由 `CodexSdkClient` 桥接。
...
- Codex SDK 相关配置统一迁移为 `AGENT_CODEX_*`，实现落地后不再读取 `SEMANTIC_MODELING_CODEX_*`。
- Codex SDK 的 `project_root`、`runtime_root` 和 workspace roots 只来自当前服务 bootstrap / 环境配置；运行时管理 API 不接受 DB extra 覆盖这些路径，避免把本项目变成跨项目 Codex control plane。
...
- `infrastructure/agent_inference_runtime` 放第三方 SDK adapter、Codex SDK client 和本地工作区实现；不再维护独立 transport 或进程管理。
...
- 补齐 `AgentInferenceRuntimeRun` 异步生命周期、Codex SDK process contract 和 artifact 权限模型。
- 补齐 `AGENT_OPENAI_*`、`AGENT_CODEX_*` 配置收敛规则和文档同步清单，不引入旧变量双读。
- 补齐 Codex SDK 本地目录运行态，明确 project / session / thread / turn / run / artifact 粒度。
...
Phase 0 是进入 implementation plan 的前置基线；后续计划必须逐项映射这些验收口径，不能直接跳到 Codex SDK 或跨模块平台化实现。
...
### Phase 4：Codex SDK 接入

- 新增 `CodexSdkRuntimeAdapter`、`CodexSdkClient` 与 `CodexSdkClientRegistry`。
- 支持 `AGENT_CODEX_PROJECT_ROOT`、`AGENT_CODEX_RUNTIME_ROOT`、`AGENT_CODEX_SANDBOX`、`AGENT_CODEX_PATH`、timeout、artifact 和 run trace。
- 按 runtime 根目录派生 project / session / thread / turn / run / artifact 目录，不再把 Codex 配置设计成普通 command provider 配置。
- 不引入多项目环境管理；各业务项目各自配置自己的 Codex SDK 环境，本项目只消费当前服务环境。
- 先只接入 `review_proposal` 和 `repair_validation_failure`。
- Codex 不参与低延迟主对话默认链路。
- 明确 SDK provider：当前主链路只通过 SDK client，不暴露底层 transport、endpoint 或启停。
- 明确对象映射：`AgentInferenceRuntimeRequest -> turn`，`AgentInferenceRuntimeRun -> run`，`AgentInferenceRuntimeResult -> result / artifact refs`。
- 明确运行生命周期：`healthcheck`、`submit_run`、`poll_run`、`cancel_run`、`stream_events`、`collect_artifacts`。
- 明确最小管理面：Phase 4 必须提供 `submit / poll / cancel / read_result / read_events / read_artifact`，不能只测 adapter 内部。
- 并发隔离：当前服务 project namespace 内按 `run_id` 创建隔离工作区；并发上限由配置控制，超限返回 `RUNTIME_POLICY_BLOCKED` 或排队。
...
- 失败恢复：SDK 不可用返回 `RUNTIME_PROVIDER_NOT_CONFIGURED` 或 `RUNTIME_PROVIDER_ERROR`；进程超时返回 `RUNTIME_TIMEOUT`；workspace 清理失败记录 warning，不影响业务状态回滚。
...
- 当前 Codex SDK live smoke：安装并认证 Codex SDK 后，设置 `AGENT_CODEX_LIVE=1`，通过 SDK client 验证 healthcheck、capabilities，以及一次 `semantic.modeling.review_proposal` 的 submit / poll / thread / event / artifact 生命周期。
- 目标扩展项：继续补 timeout、cancel、并发上限、stale recovery、artifact 越权拒绝和命令拒绝。
- 数据资产二号消费者 smoke：字段语义推断只生成候选，不写正式资产事实。
- Codex live smoke 默认 opt-in，不进入本地默认 `make verify`；CI 只跑 SDK fake、run lifecycle 和 contract 测试。
...
| Codex SDK 生命周期复杂 | 先 per-project local runtime，限定 review / repair 两类 action |
...
- 当前查询 API 的原则落点：GET 端点只读 repository，不复用 invoke/service 执行路径，避免把观测面和执行面耦合；Codex live smoke 保持 opt-in，避免为真实 SDK 依赖引入默认副作用。
...
1. Codex SDK runtime 实现平台 `AgentInferenceRuntimePort`，并支持异步 run 生命周期、artifact 权限和 opt-in live smoke。
