...
- 生成的 Cube 仍进入 `SemanticModelingAgent` 的 spec / validate / apply / publish 链路。
...
- KISS：资产底座只做元数据事实层，语义链路继续使用现有 `SemanticModelingAgent / Semantic Mapper / Execution Compiler`。
