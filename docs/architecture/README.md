...
10. [agent-runtime-platform.md](agent-runtime-platform.md)：目标设计；平台级 Agent Runtime 层统一 OpenAI Agents SDK / LLM API 与 Codex app-server，两类 runtime 可被语义、数据资产、查询、治理和数据开发模块复用
...
- [agent-runtime-platform.md](agent-runtime-platform.md)
  - 目标设计；Agent Runtime 上提为平台级能力层，通过统一 `AgentRuntimeService / RuntimeRouter / Context Pack / Tool Registry / Runtime Policy / Trace` 对接 OpenAI Agents SDK / LLM API 和 Codex app-server；语义建模 Copilot 是首个消费者，数据资产、智能查询、治理中心和数据开发助手后续复用同一 runtime contract
