...
| 期 | 内容 | 价值 / 风险 |
|---|---|---|
| **P0 骨架** ✅ | CLI bootstrap（in-process `create_app(role='worker')`）+ 输出契约（JSON envelope/`--output`）+ `--principal` 透传 + 退出码 + `describe` 自描述。落在 `app/interfaces/cli/`，入口 `python -m app.interfaces.cli` | 地基；**已落地** |
| **P1 读域** ✅ | datasource/asset/cube/ontology/manifest 的 `list/show/describe/fields/evidence`（**全只读、缓存优先、零门**） | agent 立刻能"看清"平台；**已落地**（29 单测 + 真实 docker exec e2e 全绿） |
| **P2 查询/诊断域** 🟡 | 已落地（preview-only 薄封装）：`query compile/plan/explain` + `intent route/extract/answerability` + `chat observe`。延后：`query run/execute/status`（MaxCompute/gateway/RLS dev 阻断）、`query diagnose`（写库+需聚合）、`intent eval`（脚本移植+真实 LLM） | agent 能"调试问数"；**核心已落地**（40 单测 + e2e 全绿） |
| **P3 建模/发布写域** 🟡 | 命令集已落地（薄封装 + 写三件套）：`cube draft/create/update` + `proposal` 9 命令（create→...→publish 7 步门控）+ `release list/show/rollback`。延后：`ontology upsert/publish`（次要）。**发布走完整提案管线跑 binding 门，不走绕门捷径**。班级学情 cube 实际发布（P3b）待执行 | **agent 能建模发布**；命令集已落地（50 单测 + dry-run e2e 全绿），实发待用户确认 |
