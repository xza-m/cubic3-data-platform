...
| `make verify-semantic` | 语义中心改动 | 在 backend + frontend 基线验证之上，补充语义专项回归与语义 smoke |
...
| 契约 | `make typecheck-contracts` | 当前未配置 OpenAPI / protobuf / GraphQL 一致性检查，显式 `skip` |
...
| 前端聚合 | `make test-frontend` | 串联前端单测、前端集成入口和平台回归 |
| 平台回归 | `make test-regression-platform-layout` | 登录页、壳层、仪表盘定向回归 |
| 平台回归 | `make test-regression-platform-data` | 数据源、数据集定向回归 |
| 平台回归 | `make test-regression-platform-query` | 查询中心、问数定向回归 |
| 平台回归聚合 | `make test-regression` | 串联平台定向回归 |
| 语义专项回归 | `make test-regression-semantic` | 语义中心页面、状态和视觉回归 |
| 自动化测试聚合 | `make test` | 串联 `test-unit`、`test-integration`、`test-regression` |
...
| `make verify-semantic` | 语义中心改动；在 backend + frontend 基线之外补语义专项回归与 smoke |
...
| `make semantic-layout` | 只验证语义中心布局与交互回归 |
| `make coverage-backend` | 需要完整后端覆盖率基线时单独运行；同时校验总门槛、模块均匀度和核心模块守护，不并入默认四层 |
| `make coverage-frontend` | 需要前端单元测试 coverage 基线时单独运行；同时校验前端总门槛与核心功能、实体页守护，不并入默认四层 |
| `make coverage` | 聚合前后端 coverage；用于专项验证或质量治理，不并入默认四层 |
...
- `make coverage-backend`：生成后端完整 pytest coverage 基线，并执行模块守护检查
- `make coverage-frontend`：生成前端单元测试 coverage 基线，并执行前端守护检查
- `make coverage`：聚合执行前后端 coverage 入口
- 后端 coverage 当前门槛按 [backend-coverage.md](backend-coverage.md) 维护；当前 `pytest.ini` 基线为 `--cov-fail-under=95`
- 后端 coverage 约束同时包括：
  - 总覆盖率门槛
  - 二级模块统一门槛：当前要求全部 `>=95%`
  - 核心模块守护：当前要求指定核心模块保持 `100%`
- 这些机器可执行规则由 `scripts/backend_coverage_rules.json` 定义，`make coverage-backend` 会自动执行 `scripts/checks/backend_coverage_guard.py`
- 后端 coverage 波次、模块目标和推进顺序统一记录在 [backend-coverage.md](backend-coverage.md)
- 前端 coverage 当前门槛按 [frontend-coverage.md](frontend-coverage.md) 维护；当前目标总门槛为 `90%`
- 前端 coverage 约束同时包括：
  - 总覆盖率门槛
  - 核心功能与实体页守护：当前要求指定核心页面保持 `100%`
- 这些机器可执行规则由 `scripts/frontend_coverage_rules.json` 定义，`make coverage-frontend` 会自动执行 `scripts/checks/frontend_coverage_guard.py`
...
- 它依赖前端开发服务、最新后端代码和可写语义目录
- 它会创建或更新草稿、测试数据或语义资产
- 它不保证 hermetic，也不承诺对工作区和数据零副作用
...
```bash
make verify-detect VERIFY_FILES="README.md frontend/src/App.tsx"
make verify-changed VERIFY_FILES="app/__init__.py frontend/src/App.tsx"
```
