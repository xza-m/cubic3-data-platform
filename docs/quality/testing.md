...
| 平台 Agent 推理 Runtime | `make test-platform-agent-runtime` | Agent Inference Runtime contract / router / OpenAI-compatible adapter / SQL 仓储 / 语义建模 consumer / 数据资产 consumer / 查询 API；Codex live smoke 默认 skip |
...
| `make test-platform-agent-runtime` | 平台 Agent Inference Runtime 改动的最小必跑入口；包含 `tests/integration/test_agent_runtime_api.py`、数据资产字段语义 consumer 测试与默认跳过的 Codex live smoke guard |
