...
| 平台 Agent 推理 Runtime | `make test-platform-agent-runtime` | Agent Inference Runtime contract / router / OpenAI-compatible adapter / SQL 仓储 / 语义建模 consumer / 数据资产 consumer / 查询 API；Codex WS live smoke 默认 skip，设置 `AGENT_CODEX_LIVE=1` 时连接真实本机 app-server |
...
| `make test-platform-agent-runtime` | 平台 Agent Inference Runtime 改动的最小必跑入口；包含 `tests/integration/test_agent_runtime_api.py`、数据资产字段语义 consumer 测试与 Codex WS opt-in live smoke |
...
- `test-platform-agent-runtime` 是平台 Agent Inference Runtime 专项回归入口，区别于 official Semantic Runtime 的 `test-agent-runtime`；真实 Codex app-server 验收目标是 `codex app-server --listen ws://127.0.0.1:<port>` + WebSocket client，HTTP REST 不作为 Codex app-server 集成模式
