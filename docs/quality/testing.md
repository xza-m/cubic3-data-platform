...
| `make verify-docs` | 仅文档改动 | 运行文档健康检查与 ADR-012 事实源口径守护 |
| `make verify-semantic` | 语义中心改动 | 在 Agent Runtime、统一查询执行面、语义建设工作台最小链路、backend + frontend 基线验证之上，补充语义专项 smoke |
...
| `make verify-semantic-prod-strict` | 语义平台上线前 | 先执行 `semantic-prod-env-required`，强制要求预生产 DB fingerprint、live smoke、fixture cleanup、真实 PostgreSQL concurrency 和 dw-query-gateway 执行配置，再执行生产候选闸门 |
...
| 事实源口径 | `make fact-source-guard` | 检查 ADR-012 下 Dataset、数据资产、平台交互式查询与 Gateway 正式问数的高频漂移表达 |
...
| 契约 | `make typecheck-contracts` | 生成 `/api/docs/openapi.json` 并校验 OpenAPI Agent / CLI-ready public contract |
...
| CLI 单测 | `make test-cli` | `cubic3-dp` 命令树、认证头、危险命令确认门禁与 payload 提交 |
| CLI 交付 | `make verify-cli` | `test-cli` + `build-cli`，确认命令行为与 wheel 构建均可交付 |
...
| 平台 Agent 推理 Runtime | `make test-platform-agent-runtime` | Agent Inference Runtime contract / router / OpenAI-compatible adapter / SQL 仓储 / 语义建模 consumer / 数据资产 consumer / 查询 API；Codex SDK live smoke 默认 skip，设置 `AGENT_CODEX_LIVE=1` 时验证真实 SDK healthcheck、capabilities 与 review run 生命周期 |
| Gateway 边界与治理执行链路 | `pytest tests/integration/test_openapi_docs.py tests/integration/queries/test_query_console_datasource_spi_api.py tests/integration/access/test_agent_semantic_execute_gateway.py -q` | OpenAPI 不暴露本仓执行面、查询工作台走 DataSource Adapter SPI、Agent semantic execute 提交到 `dw-query-gateway` |
| 事实源边界与聚合统计 | 受影响模块的后端 service 单测 + 对应前端 smoke | 覆盖 ADR-012：数据资产规模读 `data_asset_*`，平台 Dataset 只表达可消费资源，平台交互式查询读 `query_histories`，正式查询统计读 gateway telemetry |
| 语义建设工作台最小链路 | `make test-modeling-agent` | 内部 `SemanticModelDraftBuilder`、迁移期 Copilot session API、候选资产确认、spec 生成、保存 Proposal、发布门禁、Domain context-preview、前端工作台页面单测 |
| 语义专项 smoke | `make smoke-semantic` | 领域创建、领域发布、治理问题、数据资产底座四条真实后端浏览器烟测 + P34 Modeling Copilot mock 闭环 |
| 语义生产候选 | `make verify-semantic-prod` | 迁移 / SQL Registry / baseline / nginx build / semantic verify / live opt-in / cleanup 的生产候选闸门 |
| 语义上线前严格验收 | `make verify-semantic-prod-strict` | 环境变量 fail-fast + `verify-semantic-prod` + 真实 PostgreSQL 发布并发测试 + gateway 执行配置检查 |
...
| 可观测 | `make smoke-observability` | Gateway 可观测代理、告警评价与 telemetry client 契约 smoke |
...
| `make verify-docs` | 仅文档改动的默认交付入口；同时执行 `docs-health` 与 `fact-source-guard` |
| `make verify-cli` | CLI 改动的默认交付入口；覆盖命令行为测试与 wheel 构建 |
| `make verify-semantic` | 语义中心改动；在 Agent Runtime、Gateway 边界与治理执行链路、语义建设工作台 / Domain 上下文最小链路、backend + frontend 基线之外补语义专项 smoke |
| `make test-platform-agent-runtime` | 平台 Agent Inference Runtime 改动的最小必跑入口；包含 `tests/integration/test_agent_runtime_api.py`、数据资产字段语义 consumer 测试与 Codex SDK opt-in live smoke |
| `make verify-semantic-prod` | 语义平台生产候选；会构建 nginx 生产镜像，默认跳过 live smoke，设置 `SEMANTIC_PROD_LIVE=1` 后运行真实链路 |
| `make verify-semantic-prod-strict` | 语义平台上线前严格验收；要求预生产 DB、live smoke、fixture cleanup、真实 PostgreSQL concurrency 与 dw-query-gateway 执行配置环境变量，不允许静默 skip |
...
- `make smoke-backend`
- `make smoke-frontend`
- `make smoke-observability`（Gateway 可观测代理、告警评价与 telemetry client 契约 smoke）
...
- 其中 `domain-smoke` / `domain-publish-smoke` / `governance-issues-smoke` / `data-assets-smoke` 依赖前端开发服务、最新后端代码和可写语义目录
...
- 领域 smoke 会创建或更新草稿、测试数据或语义资产；治理问题 smoke 只读现有 Cube 与治理问题接口；数据资产 smoke 会通过资产同步 API 写入确定性 MaxCompute-like 元数据夹具，并创建真实 Modeling Copilot session 验证资产底座到 Copilot 入口的连接
...
- `e2e:modeling-agent-smoke:live` 是发布前 opt-in 补证，运行真实后端 session / Proposal / 带消费者预演角色的 release-preview / publish 链路，不并入默认验证
- Gateway 边界专项验证确保 Agent semantic execute 提交到 `dw-query-gateway`，查询工作台 / SQL Lab 保持 DataSource Adapter SPI，不恢复本仓内部执行面
- 事实源边界专项验证确保 dashboard、context panel 和健康指标不混用 `datasets`、`data_asset_*`、`query_histories` 和 gateway telemetry；聚合卡片必须能解释来源
- `preflight-agent-runtime` / `live-agent-runtime` 是真实环境 opt-in 补证：前者只检查 active 语义资产绑定，后者会进入真实 MaxCompute 执行验收，不并入默认 `make verify`
- `test-platform-agent-runtime` 是平台 Agent Inference Runtime 专项回归入口，区别于 official Semantic Runtime 的 `test-agent-runtime`；真实 Codex SDK 验收目标是 `tests/integration/agent_inference_runtime/test_codex_sdk_live_smoke.py`，覆盖 SDK healthcheck、capabilities、review run 的 thread / event / artifact 生命周期；HTTP REST / WebSocket 不作为平台 Codex 集成模式
...
`npm run e2e:data-assets-smoke` 底层执行 `frontend/tests/e2e/data_asset_foundation_smoke.py`。它是真实后端契约脚本，覆盖：

- 使用真实 JWT 调用语义 API，不接受占位 token。
- `POST /api/v1/semantic/assets/sync-runs` 写入确定性 `data-asset-smoke` 元数据夹具。
- `GET /api/v1/semantic/assets/radar` 校验表与字段统计反映本次同步。
- `GET /api/v1/semantic/assets/tables` 找回 smoke 物理表。
- `GET /api/v1/semantic/assets/tables/<id>/evidence` 校验 `EvidenceBundle` 包含 `asset_refs`、`schema_snapshot`，且 `runtime_truth=false`。
- `GET /api/v1/semantic/governance/issues?schema_source=asset_snapshot` 校验治理问题接口可切换到资产快照来源。
- `POST /api/v1/semantic/modeling-copilot/sessions` 使用“查询最近 7 天学生评论数，按学校汇总”业务问题创建 Copilot session，证明数据资产证据已可用后能接入建模助手入口；当前 session 创建契约不直接写入 EvidenceBundle，候选资产证据由 Copilot 后续召回读取。

当前完成态要求该脚本在可写测试库中通过；若出现 `HTTP 404`、鉴权失败或契约字段错误，表示数据资产底座 API / Copilot session / 权限 / E2E 契约发生回归。

#### 字段候选层专项验证

字段候选层影响物理类型映射、字段角色推断、Cube 草案生成、Schema sync 兼容策略、Copilot Review 展示和 Cube 创建入口文案。涉及 `app/application/semantic/field_candidates/`、`CubeModelingService`、`CubeModelingSourceService`、`SemanticModelDraftBuilder`、`SchemaSyncService`、`/api/v1/semantic/field-candidates/preview`、Cube 创建页或 Modeling Copilot 字段候选 trace 时，优先按下列专项命令补证：

```bash
pytest --no-cov tests/unit/application/semantic/field_candidates
pytest --no-cov tests/unit/application/semantic/test_cube_modeling_service.py tests/unit/application/semantic/test_cube_modeling_source_service.py tests/unit/application/semantic/test_modeling_draft_builder.py tests/unit/application/semantic/test_schema_sync.py
pytest --no-cov tests/integration/semantic/test_field_candidate_api.py
cd frontend && npm run test:unit -- src/v2/api/semantic-field-candidates.test.ts src/v2/pages/semantic/cubes/CubeCreate.test.tsx src/v2/pages/semantic/modeling-copilot/ModelingAgent.artifactPanel.test.tsx src/v2/pages/semantic/modeling-copilot/ModelingAgent.sessionRail.test.tsx src/v2/pages/semantic/modeling-copilot/ModelingAgent.chatFlow.test.tsx src/v2/pages/semantic/modeling-copilot/ModelingAgent.publish.test.tsx src/v2/pages/semantic/modeling-copilot/BatchModelingAgent.test.tsx src/v2/pages/semantic/modeling-copilot/batchModeling.test.ts src/v2/pages/semantic/modeling-copilot/components/BatchModelingWorkbench.test.tsx
cd frontend && npx playwright test --config tests/e2e-v2/playwright.config.ts tests/e2e-v2/p35-field-candidate-layer.spec.ts
```

最后一条 P35 E2E 是可选交付补证，适合字段候选入口文案、preview API 端到端契约或交付验收任务；默认整体验证仍回到 `make verify-semantic` 或主代理指定的更宽入口。
...
- 默认执行离线 Alembic 拓扑检查；如果设置 `DATABASE_URL`，会额外跑存量库 fingerprint 检查
...
- 最后执行 cleanup；默认无 `SEMANTIC_FIXTURE_NAMESPACE` 时跳过，设置 `SEMANTIC_FIXTURE_NAMESPACE` 后使用同一个 `DATABASE_URL`，再由 `scripts/checks/semantic_fixture_cleanup.py` 调用 `tests/support/semantic_fixture_manager.py` 按 namespace 清理测试资产

`make verify-semantic-prod-strict` 的契约是：

- 先执行 `make semantic-prod-env-required`，缺少 PostgreSQL 类型的 `DATABASE_URL`、`SEMANTIC_PROD_LIVE=1`、`SEMANTIC_FIXTURE_NAMESPACE`、`QUERY_GATEWAY_BASE_URL` 或 `QUERY_GATEWAY_PLATFORM_SERVICE_TOKEN` 时直接失败；SQLite URL 不会通过 concurrency 门禁
