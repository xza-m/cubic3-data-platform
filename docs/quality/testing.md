...
- `test-platform-agent-runtime` 是平台 Agent Inference Runtime 专项回归入口，区别于 official Semantic Runtime 的 `test-agent-runtime`；真实 Codex app-server smoke 必须显式设置 `AGENT_CODEX_LIVE=1`，并配置 `AGENT_CODEX_ENDPOINT` 或 `AGENT_CODEX_UNIX_SOCKET`
