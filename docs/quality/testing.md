...
- 最后执行 cleanup；默认无 `SEMANTIC_FIXTURE_NAMESPACE` 时跳过，设置 `SEMANTIC_FIXTURE_NAMESPACE` 与 `SEMANTIC_FIXTURE_DATABASE_URL` 后由 `scripts/checks/semantic_fixture_cleanup.py` 调用 `tests/support/semantic_fixture_manager.py` 清理测试资产
