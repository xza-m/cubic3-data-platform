...
- 先执行 `make semantic-prod-env-required`，缺少 PostgreSQL 类型的 `DATABASE_URL`、`SEMANTIC_PROD_LIVE=1` 或 `SEMANTIC_FIXTURE_NAMESPACE` 时直接失败；SQLite URL 不会通过 concurrency 门禁
