...
| 平台 Agent 推理 Runtime | `make test-platform-agent-runtime` | Agent Inference Runtime contract / router / OpenAI-compatible adapter / SQL 仓储 / 语义建模 consumer / 查询 API；Codex live smoke 默认 skip |
...
| `make test-platform-agent-runtime` | 平台 Agent Inference Runtime 改动的最小必跑入口；包含 `tests/integration/test_agent_runtime_api.py` 与默认跳过的 Codex live smoke guard |
...
- `test-platform-agent-runtime` 是平台 Agent Inference Runtime 专项回归入口，区别于 official Semantic Runtime 的 `test-agent-runtime`；真实 Codex app-server smoke 必须显式设置 `AGENT_CODEX_LIVE=1`，并配置 `AGENT_CODEX_ENDPOINT`，随后会调用 `/health` 与 `/capabilities`
