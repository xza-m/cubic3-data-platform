...
- 建立本项目内的统一查询执行能力 `query-execution`。
...
- Configurable `QUERY_WORKER_ID` and `QUERY_WORKER_CONCURRENCY`.
...
Worker fetch 阶段 MUST 使用 streaming/chunked 方式写入 result store，禁止把完整结果一次性载入内存。
...
默认最大重试次数为 3 次，使用指数退避。每次重试都必须写入 `query_execution_events`。
...
- **现有 SQL Lab 有旧执行路径**：第一版可以保留，但新实现不得再新增第二套执行面；后续逐步迁到 `query-execution`。
...
- **旧查询入口契约差异**：SQL Lab 等旧入口第一版可以双轨保留，避免同步 API 被异步协议强行打断。
...
- **DRY**：Agent、SQL Lab、后续应用执行统一复用 `query-execution`，避免重复查询执行逻辑。
