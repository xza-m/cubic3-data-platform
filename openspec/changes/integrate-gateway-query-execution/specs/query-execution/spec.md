...
系统 SHALL 在本项目内提供统一查询执行面，用于承接 Agent Runtime、SQL Lab、查询模板和后续应用执行产生的只读 SQL 查询任务。
...
#### Scenario: 手写 SQL 提交查询任务

- **WHEN** SQL Lab 或查询中心提交手写 SQL
...
- **WHEN** Worker 从数仓拉取结果
- **THEN** Worker SHALL 使用 streaming 或 chunked fetch
- **AND** 不得将完整结果一次性载入内存
...
- **AND** Worker SHALL 按最大 3 次指数退避重试
