...
- 不强制迁移 SQL Lab 的同步开发者查询体验；SQL Lab 是数据开发工具面，不是 Agent Runtime 的正式执行面。
...
现有语义诊断 API、SQL Lab、查询模板和导出能力不在本 change 中直接删除。职责边界调整为：

- Agent-first Runtime、后续应用 Runtime、自动化问数等正式运行时查询 SHALL 走新的 `query-execution` 执行面。
- SQL Lab SHALL 保留为数据开发人员使用的同步异构数据源查询工具面，通过 RBAC 限制开放范围。
- SQL Lab MAY 复用 SQL Guard、DataSourceAdapter、审计规范等底层能力，但不要求走 `ExecutionTicketSnapshot + async job + result object` 协议。
- 不得为 Agent / Runtime 新增绕过 `query-execution` 的数仓执行路径。
