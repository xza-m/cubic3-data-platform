...
class _CodexAgentApp(_Runtime):
    ...
    def start_review_proposal(self, *, session, proposal_id):
        self.review_calls.append({"session_id": session.id, "proposal_id": proposal_id})
        return {
            "run_id": "run_review_1",
    ...
    def start_repair_validation_failure(self, *, session):
        self.repair_calls.append({"session_id": session.id})
        return {
            "run_id": "run_repair_1",
...
def test_start_review_run_persists_codex_metadata_without_publishing():
    ...
    assert agent_app.review_calls == [{"session_id": "session_1", "proposal_id": "proposal_1"}]
...
def test_start_repair_run_persists_codex_metadata_without_applying_assets():
    ...
    assert proposal_service.calls == []
    assert agent_app.repair_calls == [{"session_id": "session_1"}]
    runtime_events = [event for event in result["event_log"] if event["type"] == "runtime_action"]
