...
description: 搭建/调试「问数·语义层」基础设施本身时用本 skill——对语义层做动作（建模、发布、调元数据、调试路由），而不是用问数查一个业务数值。关键：哪怕句子里有学校/年级/正确率/学情/知识点这类业务词，只要动词指向"建/发/改/调试语义层"，就用本 skill，不要因为有业务词就归给 dw-query。出现以下任一信号即触发：把表/宽表建成 cube 发布到语义层、发布前看 diagnostics；改 cube/维度的 title 或元数据并重新发布、运维 active manifest；问数覆盖不了/答不出/缺某维度、某问题 ground/路由到哪个 cube、意图怎么 ground；盘点线上高频但没建模的维度、给建模排优先级；维护本体(object/metric/glossary/relation/policy)、查治理审计；浏览语义资产(datasource/asset/cube/view/manifest)。驱动 in-process 本地 CLI semctl（python -m app.interfaces.cli，后端容器内跑）。反向（→ 用 dw-query，不用本 skill）：只想要一个具体数字或一段明细数据（多少学生、正确率多少、拉某段数据），既没提建模/发布，也没问路由/覆盖/缺维度。
---

# dp-semantic-builder：建设和运维 CUBIC3 语义平台

引导 agent 用语义平台的 CLI（本地引擎 `semctl` / 远程客户端 `cubic3-dp`）端到端**建设和运维语义层**：读资产 → 建模 → 发布 cube → 调试问数 → 维护本体。CLI 薄封装平台既有 application 服务。

## 何时用本 skill / 何时不用

| 用户想要 | 用 |
|---|---|
| 建模、发布 cube、维护本体、把表发进 manifest | **dp-semantic-builder** |
| 调试问数：为什么这么路由、能不能答、缺什么维度、意图怎么 ground | **dp-semantic-builder** |
| 看语义资产/manifest、查治理审计（只读） | **dp-semantic-builder** |
...
- **先 `describe` 看命令目录**（两入口都有；远程 `cubic3-dp describe` 看不 boot app）。`schema <group> <cmd>` 看单命令参数契约（仅 semctl）。

## 依赖自举（仅远程 cubic3-dp 入口需要）

semctl 随后端镜像走、无需自举。远程入口用前按序自检：
1. **CLI 存在**：`command -v cubic3-dp`；不存在则 `npm i -g @cubic3/dp-cli --registry https://verdaccio.xiaoluxue.cn/`（需 Node 18+）。
2. **登录态**：`cubic3-dp --base-url <平台> auth whoami`；未登录则 `cubic3-dp --base-url <平台> auth login`（或带 `--access-token <jwt>` / `--api-key`）。
3. **能力发现**：`cubic3-dp describe`（输出命令目录 + 契约；远程只覆盖 T1，写域走 semctl）。
注：cubic3-dp 无 `--version` flag；版本看 `npm ls -g @cubic3/dp-cli`。
...
🌐 query     compile/plan/explain        # 语义编译/规划（preview-only，不出数；plan/explain 带 --runtime-mode）
🌐 intent    route/extract/answerability # 问数调试：路由/意图/可回答性（均带 --runtime-mode official|preview）
🌐 chat      observe                     # 观察 DataChat 问数（结果分布+缺口维度；远程 HTTP 需 admin，本地 semctl 无此门）
🔒 proposal  create/confirm-source/update-spec/draft/validate/gap/approve/apply/publish  # 门控发布管线（9 命令；典型 7 步，跳 confirm-source/draft；gap 只读）
🌐 release   list/show                   # 发布读
🔒          rollback                     # 回滚 live manifest 安全网
🔒 schema    <group> [<cmd>]             # 命令参数自描述（仅 semctl；远程用 cubic3-dp describe）
```

🔒 写域命令在 cubic3-dp 上**命令存在**、敲了会返回清晰的 `local_only` 指引（不是"未知命令"），真实执行用 `python -m app.interfaces.cli`（exec 进部署）。
...
`intent extract "<问题>" --runtime-mode official` → 看 L1 grounded 意图；
...
（治理目前只有审计只读 `governance audit list/get`；授权写 grant/scope/policy 尚未实现。）
